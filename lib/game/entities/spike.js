...
.defines(function(){
	...
	ig.EntitySpike = ig.global.EntitySpike = ig.Creature.extend({
		...
		kill: function() {
			...
			if (!ig.game.player.achievements.killed_spike.achieved) {
				ig.game.player.achievements.killed_spike.achieved = true;
				ig.game.myNoteMgr.spawn(_c.PATH_TO_MEDIA+'font_helloplusplus_white_8.png', ig.game.player.achievements.killed_spike.copy, ig.system.width, ig.system.height, { lifetime: 2.0, vel: { x: 0, y: -5 } });
				//ig.game.myNoteMgr.spawn(_c.PATH_TO_MEDIA+'font_helloplusplus_white_8.png', ig.game.player.achievements.killed_spike.copy, ig.game.camera.screenLast.x, ig.game.camera.screenLast.y, { lifetime: 0, vel: { x: 0, y: 0 } });
			}
